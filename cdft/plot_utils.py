...
def configure_4panels(zbins):
    fig, ax = plt.subplots(2, 2, figsize=(8, 6.5), sharex=True)
    ax[0,0].set_xlim(zbins[0], zbins[-1])
    ax[0,1].set_ylim(-17, 2)
    ax[1,0].set_ylim(-0.003, 0.003)
    ax[1,1].set_ylim(-15, 15)
    
    ax[0,0].set_ylabel(r'$\rho(z)$ [$\mathrm{\AA}^{-3}$]')
    ax[0,1].set_ylabel(r'$\beta[\mu - V_{\mathrm{ext}}(z) - q \phi(z)]$')
    ax[1,0].set_ylabel(r'$n(z)$ [$e\mathrm{\AA}^{-3}$]')
    ax[1,1].set_ylabel(r'$-\beta e \Delta \phi(z)$')
...
def plot_interactive_4panels(fig, ax, zbins, rho_H, rho_O, charge_density, 
                             restructuring_potential, muloc_H, muloc_O, color_count):
    display.clear_output(wait=True)
    ax[0,0].plot(zbins, rho_H, color=color_cycle[color_count] )
    ax[0,0].plot(zbins, rho_O, ls="--", color=color_cycle[color_count])
...
def plot_end_4panels(zbins, rhoH, rhoO, mulocH, mulocO, ax):
    display.clear_output(wait=False)
    ax[0,0].plot(zbins, rhoH, color='black', lw=2)
    ax[0,0].plot(zbins, rhoO, ls='--', color='black', lw=2)
    ax[0,1].plot(zbins, mulocH, color='black', lw=2)
    ax[0,1].plot(zbins, mulocO, ls='--', color='black', lw=2)
